...
export async function runReadme(dir: string = process.cwd()): Promise<void> {
  ...
  // Detect stub READMEs (fallback template) and regenerate from scratch
  let isStub = false;
  if (hasReadme) {
    try {
      const existing = await Bun.file(readmePath).text();
      isStub = existing.includes("README generated by GitPal \u2014 update this with a real description");
    } catch { /* ok */ }
  }

  let result: string;
  if (!hasReadme || isStub) {
    gp.info(isStub ? "Stub README detected \u2014 regenerating from scratch..." : "No README found \u2014 generating from scratch...");
